...
class Manager(base.ResourceBase):
    ...
    def bmc(self):
        ...
        itself, structurally identical to BIOS settings -- an ``Attributes``
        dictionary described by an ``AttributeRegistry``. This exposes the
        attributes carried directly on the Manager resource, as with the
        standard ``@Redfish.Settings`` pattern.

        Redfish does not standardise where a Manager exposes these, and many
        vendors put them in an OEM sub-resource instead (for Dell iDRAC, under
        ``Links/Oem/Dell/DellAttributes``); those are reached through the
        matching OEM extension (``get_oem_extension``) rather than here.
        ...
        :raises: MissingAttributeError if the Manager does not expose
            ``Attributes`` directly.
        """
        if self.json.get('Attributes') is None:
